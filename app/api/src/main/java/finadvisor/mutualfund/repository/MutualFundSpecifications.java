    ...
    public static Specification<MutualFund> matching(String search, String amc, String category, String subCategory,
        ...
        return (root, query, cb) -> {
            ...
            if (planType != null && !planType.isBlank()) {
                predicates.add(cb.equal(root.get("planType"), finadvisor.mutualfund.entity.PlanType.valueOf(planType.trim().toUpperCase())));
            }
            if (optionType != null && !optionType.isBlank()) {
                predicates.add(cb.equal(root.get("optionType"), finadvisor.mutualfund.entity.OptionType.valueOf(optionType.trim().toUpperCase())));
            }
            if (riskLevel != null && !riskLevel.isBlank()) {
                predicates.add(cb.equal(root.get("riskLevel"), finadvisor.mutualfund.entity.FundRiskLevel.valueOf(riskLevel.trim().toUpperCase())));
            }
            if (minAum != null) {
